...
boxplot(formula = bwt ~ smoke, # y ~ x
        data = birthwt |> dplyr::mutate(smoke = factor(smoke, labels = c("Nonsmoker", "Smoker"))), 
        ylab = "Birthweight (g)", 
        xlab = "Smoking status")
...
## Exercise: `ggplot` {.smaller .scrollable}

:::{.xsmall}
```{code-blanks-instructions}
Create a boxplot of birthweight by race. First, set race as a factor with labels. Title the boxplot "Boxplot of birthweight by race" and increase the font size of the x- and y-axis labels.
```
