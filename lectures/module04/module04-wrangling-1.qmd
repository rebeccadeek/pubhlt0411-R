...
![](figs/01_masking.png){fig-alt="Console output from loading the tidyverse package in R with a startup message listing the core packages attached (dplyr, forcats, ggplot2, lubridate, purrr, readr, stringr, tibble, tidyr) with their version numbers, followed by a Conflicts section noting that dplyr's filter() and lag() functions mask the same-named functions from the stats package, and a tip about using the conflicted package to turn conflicts into errors." height=2in fig-align=center}
...
:::{.column width="50%"}
![](figs/02a_MagrittePipe.jpg){fig-alt="Painting of a brown tobacco pipe with the handwritten French caption 'Ceci n'est pas une pipe' ('This is not a pipe') below it. René Magritte's The Treachery of Images (1929)." fig-align=center}
:::

:::{.column width="50%"}
![](figs/02b_pipe.png){fig-alt="Hexagonal badge with the R 'pipe' operator symbol '%>%' displayed, and the handwritten caption 'Ceci n'est pas un pipe' ('This is not a pipe') below, a pun on Magritte's painting, referencing the magrittr R package." height=3in fig-align=center}
:::
...
# keep older than 25 with history of hypertension and save as new object
birthwt.over25.ht = birthwt |> dplyr::filter(age > 25, ht == 1)
birthwt.over25.ht
...
birthwt |> 
  dplyr::distinct(age, smoke) |> 
  dplyr::arrange(age, smoke)
...
- More than 2? Try `dplyr::case_when()` (see Appendix).
...
## Resolving the `group_by` warning {.smaller}
::: notes
...
birthwt |> 
  dplyr::summarise(.by = c(smoke, race), # alt to group_by
                   mean.bwt = mean(bwt), sd.bwt = sd(bwt) )
...
birthwt |> 
  ...
  dplyr::summarise(mean.bwt = mean(bwt), sd.bwt = sd(bwt), 
                   .groups = "keep") # keep groups after computation ("drop" also possible)

```

# Appendix {visibility="uncounted"}

## `mutate()` + `case_when()` {.smaller visibility="uncounted"}
::: notes
- logical reads as "if"
- ~ reads as "then"
- , reads as "else"
...
birthwt.clean = birthwt |> 
  dplyr::mutate(
    ...
    race = dplyr::case_when( # this will write over the original race var
      race == 1 ~ "White", 
      race == 2 ~ "Black", 
      race == 3 ~ "Other", 
      .default = NA
...
## `select()` + `where()` {.smaller visibility="uncounted"}
- Select a variable based
