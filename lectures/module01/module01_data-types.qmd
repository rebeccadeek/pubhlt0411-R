...
blood.types.numb = c(1, 2, 1, 4, 3, 3)
...
- We can coerce back to numeric (but doesn't always work).
...
## Coercing the type of objects

- Coercion does not always work in the way we expect.

```{r type-coercion4}

eye.color = c("black", "blue", "brown", "green", "gray", "hazel")
...
- Factors are used for variables that have a fixed and known set of possible values.
  - Useful when character vectors have a specific order.
...
![](_figs/02_scal_vec.png){fig-alt="Diagram comparing a scalar, shown as a single small square, to a vector, shown as a column of five stacked squares." width=35% fig-align=center}
...
![](_figs/03_mat_array.png){fig-alt="Diagram comparing a matrix, shown as a single 5x5 grid, to an array, shown as multiple stacked 5x5 grids offset diagonally to represent higher dimensions." width=65% fig-align=center}
...
- Generalizations of `names()`: `rownams()` and `colnames()`.
...
- Binding the rows (vertically stacking) can be helpful for combining two matrices.
...
sq.mat1.df # name automatically assigned matrix -> df
